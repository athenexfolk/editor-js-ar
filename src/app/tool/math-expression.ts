...
interface MathExpressionData extends BlockToolData {
  mathText: string;
}

export default class MathExpression implements BlockTool {
  data: MathExpressionData = {
    mathText: '',
  };
  ...
  constructor({
    ...
    api,
    readOnly,
  }: BlockToolConstructorOptions<MathExpressionData>) {
    this.api = api;
  ...
  drawView() {
    ...
    if (!this.readOnly) {
      ...
      this._input.placeholder = 'Type Katex Expression...';
  ...
  renderKatex(data: string) {
    let renderText = katex.renderToString(data, {
      throwOnError: false,
      displayMode: true,
    });
    this._display.innerHTML = renderText;
  }

  validate(savedData: MathExpressionData) {
  ...
  static get toolbox() {
    return {
      icon: '<p style="font-family:monospace;font-weight-bold;font-size: 16px;">M</p>',
